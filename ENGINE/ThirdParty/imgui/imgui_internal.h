...
struct IMGUI_API ImBitArray
{
	...
	{
		IM_ASSERT(n < BITCOUNT); return ImBitArrayTestBit(Storage, n);
	}
	void            SetBit(int n)
	{
		IM_ASSERT(n < BITCOUNT); ImBitArraySetBit(Storage, n);
	}
	void            ClearBit(int n)
	{
		IM_ASSERT(n < BITCOUNT); ImBitArrayClearBit(Storage, n);
	}
...
struct IMGUI_API ImPool
{
	...
	{
		Buf[idx].~T(); *(int *)&Buf[idx] = FreeIdx; FreeIdx = idx; Map.SetIntUniform(key, -1); AliveCount--;
	}
...
struct ImGuiNavItemData
{
	...
	void Clear()
	{
		Window = NULL; ID = FocusScopeId = 0; RectRel = ImRect(); DistBox = DistCenter = DistAxial = FLT_MAX;
	}
};
