...
CCollider::CCollider(Transform *parent, GameObject *parentObj)
	:CComponent{ parent, parentObj }, m_allowRotation{ true }, m_active{ true }, m_shape{ Shape::None }
{
...
void CCollider::AddSphereCollider()
{
	if (this->GetParentObject()->GetComponent<CStaticMesh>() != nullptr)
	{
		...
		m_XHalfSize = (minMax[1] - minMax[0]) / 2.0f;
		m_YHalfSize = (minMax[3] - minMax[2]) / 2.0f;
		m_ZHalfSize = (minMax[5] - minMax[4]) / 2.0f;
	...
	{
		std::cout << "CCollider::AddSphereCollider() function. Cannot autosize because GameObject does not have a CstaticMesh!\n";
		exit(-24);
	}

	float biggest = m_XHalfSize;
	if (m_YHalfSize > biggest)
	{
		biggest = m_YHalfSize;
	}
	if (m_ZHalfSize > biggest)
	{
		biggest = m_ZHalfSize;
	}
...
void CCollider::AddBoxCollider(float x, float y, float z, float offsetX, float offsetY, float offsetZ, bool autoSize, int layer, bool allowRotation, int colMask)
{
	...
	m_colObj->setCollisionFlags(btCollisionObject::CF_DYNAMIC_OBJECT);
	COLLISION->RegisterCollisionBody(m_colObj, this);
	btBoxShape *boxShape = new btBoxShape(btVector3(m_XHalfSize, m_YHalfSize, m_ZHalfSize));
